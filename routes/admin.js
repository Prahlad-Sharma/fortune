  ...
  router.put('/deposits/:id', adminMiddleware, async (req, res) => {
    try {
      ...
      if (action === 'approve') {
        ...
        if (isWithdraw) {
          await db.query('UPDATE users SET coins=GREATEST(0,coins-?) WHERE id=?', [dep.amount, dep.user_id]);
          await db.query('INSERT INTO transactions (user_id,type,amount,coin_type,description) VALUES (?,?,?,?,?)', [dep.user_id, 'loss', dep.amount, 'real', `📤 Withdrawal Approved ₹${dep.amount}`]);
        } else {
      ...
      res.json({ success: true, message: action === 'approve' ? 'Approved!' : 'Rejected' });
    } catch(e) { res.json({ success: false, message: e.message }); }
  });
